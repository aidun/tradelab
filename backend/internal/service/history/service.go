...
func (s *Service) ListOrders(ctx context.Context, walletID string, limit int, marketSymbol string, mode domain.AccountingMode) ([]domain.Order, error) {
	...
	annotated := tradingcalc.AnalyzeOrders(orders, domain.NormalizeAccountingMode(string(mode)), map[string]float64{}).Orders
	if marketSymbol != "" {
		filtered := make([]domain.Order, 0, len(annotated))
		for _, order := range annotated {
			if order.MarketSymbol == marketSymbol {
	...
	if s.logger != nil {
		s.logger.Info("list_order_history.success", "operation", "list_order_history.success", "wallet_id", walletID, "limit", limit, "result_count", len(annotated), "accounting_mode", mode)
	}

	return annotated, nil
}

func (s *Service) ListActivity(ctx context.Context, walletID string, limit int, marketSymbol string) ([]domain.ActivityLog, error) {
	...
	if marketSymbol != "" {
		filtered := make([]domain.ActivityLog, 0, len(activity))
		for _, item := range activity {
			if item.MarketSymbol == marketSymbol {
